...
const AllServiceProviders = () => {
  ...
  const getSps = async () => {
    ...
    const response = await Get(
      BaseURL(`users/get/service-providers?page=${page}&limit=${recordsLimit}`)
    );
  ...
  return (
    <>
      <PageHeader item={{ bold: "Service Providers" }} />
                ...
                <Row>
                  {sps?.map((e, key) => (
                    <Col
                      ...
                      key={key}
                      // onClick={() => {
                      //   navigate(`/service/${e?._id}`);
                      // }}
                    >
                      <ServiceProviderCard item={e} />
                    </Col>
                  ))}
                </Row>
              ) : (
                <NoData text={"No Service Provider Found"} />
              )}
            </Container>
