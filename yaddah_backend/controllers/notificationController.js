...
exports.getAllNotificationsForAll = catchAsync(async (req, res, next) => {
  ...
  const notfs = await Notification.find({
    ...
    .populate('sender', 'firstName lastName email photo description isOnline')
    // .populate('receiver')
    .sort({ createdAt: -1 })
    .skip(skip)
    .limit(limit);
...
exports.createNotification = async (pushNotificationObject, obj) => {
  ...
  if (pushNotificationObject?.fcmToken?.length > 0) {
    ...
    //   .sendMulticast({
    //     notification: {
    //       title: pushNotificationObject?.title || 'Yaddah!!',
    //       body: obj.message,
    //       // imageUrl,
    //       imageUrl:
    //         'https://i.pinimg.com/736x/f0/42/ad/f042ada5fe30d167bc6a9b0c0fc0a60e.jpg',
    //     },
    //     ...(pushNotificationObject?.data && pushNotificationObject?.data),
